...
class ContractDocument(SellingController):
    # def __init__(self, *args, **kwargs):
    # 	super(ContractDocument, self).__init__(*args, **kwargs)
    ...
    def validate_warehouse(self):
        super(ContractDocument, self).validate_warehouse()
    ...
    def validate_with_previous_doc(self):
        super(ContractDocument, self).validate_with_previous_doc(
    ...
    def on_cancel(self):
        ...
        # super(ContractDocument, self).on_cancel()
...
def create_sales_invoice(source_name, target_doc=None):
    ...
    for x in frappe.flags.args.item:
        ...
        doc.append("items",{
                        ...
                        "item_row":x.get("row_name"),
                        "description": x.get("description")
                
            } )
        
    for x in frappe.flags.args.tax:
       
        doc.append("taxes",{
                    "charge_type": x.get("charge_type"),
					"account_head": x.get("account_head"),
					"description":x.get("description"),
					"rate": x.get("rate"),
					"tax_amount": x.get("tax_amount"),
                    "party_type": x.get("party_type"),
					"party": x.get("party"),
					"total":x.get("total"),
					"cost_center":x.get("cost_center"),
					"tax_amount_after_discount_amount": x.get("tax_amount_after_discount_amount"),
					"base_tax_amount_after_discount_amount": x.get("base_tax_amount_after_discount_amount"),
					"base_tax_amount":x.get("base_tax_amount"),
					"base_total": x.get("base_total"),
...
def create_purchase_invoice(source_name, target_doc=None):
    ...
    doc.due_date = frappe.utils.today()
    doc.supplier = frappe.flags.args.supplier
    doc.project = frappe.flags.args.project
    doc.contract_document = frappe.flags.args.name
